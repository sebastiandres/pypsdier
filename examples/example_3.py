...
################################################################################
inputs = {}
inputs["SeedFile"] = "example_3.rde" # filename where the simulation will be stored
inputs["SimulationTime"] = 5. # [s], total time to be simulated 
inputs["SavingTimeStep"] = 1. # [s], saves only one data per second
inputs["CatalystVolume"]  = 2.0 # [mL], total volume of all catalyst particles in reactor
...
inputs["InitialConcentrations"] = (200., 0., 0., 600.) # [mM], initial concentration of substrates and products
...
inputs["CatalystParticleRadius"] = (341.0E-6,) # [m], list of possible catalyst particle radiuses
...
SI.simulate("ode")
SI.simulate("pde")
SI.save("example_3.rde")
#SI.plot(filename="example_0.png")
SI.export_xls("example_3.xls")